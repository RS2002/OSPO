...
class Platform():
    ...
    def feedback(self, observe, current_order_state, current_order_num, assignment, new_orders_state, time_threshold, reward_func, current_time):
        ...
        old_std = np.std(self.total_reward_list)
        ...
        for i in range(len(results)):
            ...
            if result[0] is None:
                ...
                reward += result[0][1]
                reward_list.append(result[0][1])
                self.total_reward_list[i] += result[0][1]
        ...
        new_std = np.std(self.total_reward_list)
        delta_std = new_std - old_std
        punishment = - 1 * delta_std

        self.reward_std_list.append(new_std)
        ...
        for i in range(len(results)):
            ...
            #     result[0][1] = (result[0][1] - reward_mean) / reward_std + punishment
            feedback_table.append(result[0])


        return feedback_table, new_route_table ,new_route_time_table ,new_remaining_time_table ,new_total_travel_time_table, new_detour_table, assignment, reward, punishment

...
def get_args():
    ...
    parser.add_argument('--hour', type=int, nargs='+', default=[14,17,20,21,22])
...
def main():
    ...
    for hour in hour_list:
        ...
        order_amount = len(demand.filtered_demand)
        ...
        for t in pbar:
            ...
            worker.update(feedback_table, new_route_table, new_route_time_table, new_remaining_time_table,
                          new_total_travel_time_table, new_detour_table, reward, punishment, assignment, order_state,
                          q_ori, t, (t == args.max_step - 1), 0)
            assignment = [x for x in assignment if x is not None]
            demand.pickup(assignment)
            demand.update()

        Pickup_Num = platform.Pickup_Num
        Service_Rate = Pickup_Num / order_amount
        ...
        log = f"Order {order_amount} | Reward: {reward}, Served Rate: {Service_Rate} , Delivery Time: {Delivery} , Detour Time: {Detour} , Pickup Time: {Pickup} , Confirmation Time: {Confirmation}"
        print(log)
        with open("eval_result.txt", 'a') as file:
            file.write(log + "\n")

...
def get_args():
    ...
    parser.add_argument('--buffer_episode', type=int, default=1000)
    ...
    parser.add_argument('--kl_mode', type=int, default=2)
...
def main():
    ...
    while True:
        ...
        for t in pbar:
            q_value, order_state, q_ori = worker.observe(demand.current_demand, t, exploration_rate)
            assignment, _ = assign(q_value)
            feedback_table, new_route_table, new_route_time_table, new_remaining_time_table, new_total_travel_time_table, new_detour_table, assignment, reward, punishment = \
            ...
            worker.update(feedback_table, new_route_table ,new_route_time_table ,new_remaining_time_table ,new_total_travel_time_table, new_detour_table, reward, punishment, assignment, order_state, q_ori, t, (t==args.max_step - 1), j)
            assignment = [x for x in assignment if x is not None]
            demand.pickup(assignment)
            demand.update()
        buffer.process()
        loss = worker.train(args.batch_size,args.train_times,kl_mode=args.kl_mode,flag=flag)
        ...
        if j % args.eval_episode == 0 :
            ...
            pbar = tqdm.tqdm(range(args.max_step))
            for t in pbar:
                q_value, order_state, q_ori = worker.observe(demand.current_demand, t)
                assignment, _ = assign(q_value)
                feedback_table, new_route_table, new_route_time_table, new_remaining_time_table, new_total_travel_time_table, new_detour_table, assignment, reward, punishment = \
                ...
                worker.update(feedback_table, new_route_table, new_route_time_table, new_remaining_time_table,
                              new_total_travel_time_table, new_detour_table, reward, punishment, assignment, order_state, q_ori, t, (t == args.max_step - 1), j)
                assignment = [x for x in assignment if x is not None]
                demand.pickup(assignment)
                demand.update()
